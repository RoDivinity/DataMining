...
data <- read.csv(url(data_file), stringsAsFactors=TRUE)
n = nrow(data)
...
sccvl = cv.gamlr(scx, scy, verb=TRUE)
...
#to assess whether the "green certification" effect is different for different buildings, or instead whether 
# it seems to be roughly similar across all or most buildings.
